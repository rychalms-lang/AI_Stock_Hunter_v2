...
OUTPUT_TRADES = f"{PERFORMANCE_DIR}/realistic_walk_forward_trades.csv"
OUTPUT_EQUITY = f"{PERFORMANCE_DIR}/realistic_walk_forward_equity.csv"
...
US_ROUND_TRIP_FX_COST = 0.02
CANADA_ROUND_TRIP_COST = 0.00
SLIPPAGE_ROUND_TRIP = 0.0025
...
def trade_cost_for_ticker(ticker):
    if is_canadian_ticker(ticker):
        return CANADA_ROUND_TRIP_COST + SLIPPAGE_ROUND_TRIP

    return US_ROUND_TRIP_FX_COST + SLIPPAGE_ROUND_TRIP
...
def main():
    if not os.path.exists(TRAINING_FILE):
        print("No historical training file found. Run historical_trainer.py first.")
        return
    ...
    print("Running realistic walk-forward backtest...")
    print(f"Strategy: Top {TOP_N}, Hold {HOLD_DAYS}, RiskOn={RISK_ON_ONLY}, ExtremeFilter={EXCLUDE_EXTREME}")
    print("Execution: signal today, enter next trading day, exit after hold period")
    print("Costs: US stocks 2% FX round trip + 0.25% slippage; Canadian stocks 0% FX + 0.25% slippage")
    print()
    ...
    for current_date in all_dates:
        ...
        for _, pick in picks.iterrows():
            ...
            total_cost = trade_cost_for_ticker(pick["ticker"])
            net_return = future_return - (total_cost * 100)
            ...
            open_trades.append({
                ...
                "gross_return_pct": round(future_return, 2),
    ...
    benchmark_start = str(pd.to_datetime(equity["date"].min()).date())
    benchmark_end = str(pd.to_datetime(equity["date"].max()).date())

    spy = get_benchmark_return("SPY", benchmark_start, benchmark_end)
    qqq = get_benchmark_return("QQQ", benchmark_start, benchmark_end)
    ...
    if spy:
        print(f"SPY final value: ${spy['final_value']:,.2f} | Return: {spy['total_return_pct']}%")
        print(f"Alpha vs SPY: {total_return - spy['total_return_pct']:.2f}%")

    if qqq:
        print(f"QQQ final value: ${qqq['final_value']:,.2f} | Return: {qqq['total_return_pct']}%")
        print(f"Alpha vs QQQ: {total_return - qqq['total_return_pct']:.2f}%")
